...
public class BannerLayoutActivity extends AppCompatActivity {
    ...
    @BindView(R.id.bannerLayoutRv)
    RecyclerView bannerLayoutRv;
    ...
    private void initTangram() {
        ...
        tangramEngine.register(BannerSupport.class, new BannerSupport() {
            @Override
            public void registerPageChangeListener(BannerListener listener) {
                super.registerPageChangeListener(listener);
            }
        });
        tangramEngine.bindView(bannerLayoutRv);
        bannerLayoutRv.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                tangramEngine.onScrolled();
            }
        });
        byte[] bytes = Utils.getAssertsFile(this, "bannerLayout.json");
        if (bytes != null) {
