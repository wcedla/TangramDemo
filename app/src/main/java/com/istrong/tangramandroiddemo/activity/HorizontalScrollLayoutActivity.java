...
public class HorizontalScrollLayoutActivity extends AppCompatActivity {

    @BindView(R.id.horizontalLayoutRv)
    RecyclerView horizontalLayoutRv;
    ...
    private void initTangram() {
        ...
        tangramEngine.bindView(horizontalLayoutRv);
        horizontalLayoutRv.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                tangramEngine.onScrolled();
            }
        });
        byte[] bytes = Utils.getAssertsFile(this, "horizontalLayout.json");
        if (bytes != null) {
