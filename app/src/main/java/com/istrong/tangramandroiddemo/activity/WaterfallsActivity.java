...
public class WaterfallsActivity extends AppCompatActivity {
    ...
    @BindView(R.id.waterfallsLayoutRv)
    RecyclerView waterfallsLayoutRv;
    ...
    private void initTangram() {
        ...
        tangramEngine.bindView(waterfallsLayoutRv);
        waterfallsLayoutRv.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                tangramEngine.onScrolled();
            }
        });
        byte[] bytes = Utils.getAssertsFile(this, "waterfallsLayout.json");
        if (bytes != null) {
