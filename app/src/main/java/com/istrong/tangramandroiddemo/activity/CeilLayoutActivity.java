...
public class CeilLayoutActivity extends AppCompatActivity {

    @BindView(R.id.ceilLayoutRv)
    RecyclerView ceilLayoutRv;
    private TangramEngine tangramEngine;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_ceil_layout);
    ...
    private void initTangram() {
        ...
        tangramEngine.bindView(ceilLayoutRv);
        ceilLayoutRv.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                tangramEngine.onScrolled();
            }
        });
        byte[] bytes = Utils.getAssertsFile(this, "ceilLayout.json");
        if (bytes != null) {
